import os
import time
import sys

os.system("clear")
os.system("cd && cd AllHuntingTools && clear && bash Logo.sh")

print("  \033[1;34m[ 01 ] >> \033[1;36;40mFacebook - Bruteforce attack on Facebook account using python")
print("  \033[1;34m[ 02 ] >> \033[1;36;40mSherlock - Hunt social media accounts by username across social networks")
print("  \033[1;34m[ 03 ] >> \033[1;36;40mUserFinder - Find UserName in social")
print("  \033[1;34m[ 04 ] >> \033[1;36;40mExit System - Log out AllHuntingTools")
print("  \033[1;34m[ 05 ] >> \033[1;36;40mBack To MainMenu")

op=int(raw_input("Soc1alF1sh: "))

if(op==1):
 os.system("clear")
 os.system("cd && cd AllHuntingTools && cd Facebook-BruteForce && python3 fb.py or python fb2.py")
elif(op==2):
 os.system("clear")
 os.system("cd && cd AllHuntingTools && cd Files && bash SherlockUser.sh")
elif(op==3):
 os.system("clear")
 os.system("cd && cd AllHuntingTools && cd UserFinder && bash UserFinder.sh && cd && cd AllHuntingTools && python3 src/Timer3.py")
elif(op==4):
 time.sleep(0.2)
 print("\033[1;31;40mQuiting System..")
elif(op==5):
 os.system("cd")
 os.system("cd AllHuntingTools")
 os.system("python2 MainMenu.py")
else:
 print("\033[1;31;40mInvalid input. Reloading Tools") 
 time.sleep(1.6)
 os.system("cd")
 os.system("cd AllHuntingTools")
 os.system("python2 Files/SocialMenu.py")
